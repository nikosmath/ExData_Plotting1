...
png(file = "~/Desktop/ExData_Plotting1/figure/plot4.png",height=480,width=480, bg = "white")

# Create the 4 graph view orientation
par(mfrow=c(2,2))

# Add the first of the four graphs 
plot(hpc_Data$Time_N,hpc_Data$Global_active_power,type='l',
     ylab='Global Active Power',
     xlab='')

# Add the second graph
plot(hpc_Data$Time_N,hpc_Data$Voltage,type='l',
     ylab='Voltage',
     xlab='datetime')


# Add the third graph
plot(hpc_Data$Time_N,ann=F,hpc_Data$Sub_metering_1,type='l')
lines(hpc_Data$Time_N,hpc_Data$Sub_metering_2,type='l',col='blue')
lines(hpc_Data$Time_N,hpc_Data$Sub_metering_3,type='l',col='red')
title(" ",xlab="",ylab="Energy sub metering")
legend('topright', colnames(hpc_Data)[7:9] ,lty=1, 
       col=c('black', 'blue', 'red'),
       bty='n', cex=.75)


# Add the fourth graph
plot(hpc_Data$Time_N,hpc_Data$Global_reactive_power,type='l',xlab='datetime',
     ylab= colnames(hpc_Data)[4])
